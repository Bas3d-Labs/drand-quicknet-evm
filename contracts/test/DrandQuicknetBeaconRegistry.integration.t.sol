...
contract DrandQuicknetBeaconRegistryIntegrationTest
    is RegistryTestBase
{
    ...
    function test_RealOracleAcceptsCompressedSignature()
        public
    {
        ...
        assertEq(
            registry.getBeacon(ROUND_1),
            normalized
        );
    }

    function test_RealOracleAcceptsUncompressedSignature()
        public
    {
        bytes memory signature =
            _sig1Uncompressed();
    ...
    function test_CompressedAndUncompressedNormalizeIdentically()
        public
    {
        ...
        DrandQuicknetBeaconRegistry
            uncompressedRegistry =
                new DrandQuicknetBeaconRegistry(
                    oracleAddress
                );

        bytes32 fromCompressed =
            compressedRegistry.submitBeacon(
                ROUND_1,
                compressed
            );

        bytes32 fromUncompressed =
            uncompressedRegistry.submitBeacon(
                ROUND_1,
                uncompressed
            );
    ...
    function test_LiveKnownVectorMatchesExpectedHash()
        public
    {
        bytes memory signature =
            _liveCompressedSignature();
    ...
    function test_SecondKnownVectorVerifies()
        public
    {
        bytes memory signature =
            _sig2Uncompressed();
    ...
    function test_SignatureForDifferentRoundRejected()
        public
    {
        bytes memory signature =
            _sig1Compressed();
        ...
        assertFalse(
            registry.isStored(wrongRound)
        );
    }

    function test_MalformedSignatureCannotStoreBeacon()
        public
    {
    ...
    function test_BitFlippedSignatureCannotStoreBeacon()
        public
    {
        bytes memory signature =
            _sig1Compressed();
        ...
        signature[20] =
            bytes1(
                uint8(signature[20]) ^ 0x01
            );

        (bool success,) =
            address(registry).call{gas: 1_000_000}(
                abi.encodeWithSelector(
                    DrandQuicknetBeaconRegistry
                        .submitBeacon
                        .selector,
